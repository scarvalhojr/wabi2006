...
The deposition sequence is often a repeated
...
illumination during mask exposure steps are minimized. This problem appears to
...
problem that is known to be NP-hard. \ignore{This formulation
...
have a single predefined embedding. If we consider all possible embeddings, the
problem is even harder (up to several million embeddings may exist for a typical
Affymetrix probe). For this reason, the problem has been traditionally tackled in two
...
The only partitioning algorithm that we know of is the Centroid-based
...
The OSPE algorithm never increases the amount of conflicts in a region.
...
with respect to its neighboring probes whose embeddings are fixed. It was originally
designed for border length minimization; here we present a formulation for conflict
index minimization.
...
The first column of $D$ is initialized as follows: $D[0,0] = 0$ and $D[i,0] = \infty$
for $0 < i \leq \ell$.
