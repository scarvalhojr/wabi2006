...
The OSPE algorithm can never increase the amount of conflicts of a region.
...
Perhaps more signficantly, we have designed our algorithm to seek the minimization
of border length or conflict indices.
...
algorithm; they are also used to partition the set of probes $\mathcal{P}$. Their
selection, however, is restricted to the probes having the least number of
embeddings. The reasons are two-fold. First, less time is spent choosing the pivots
...
and are thus a better choice for pivots.
...
bases}, which sometimes forces the selection of the probes with the next minimum
...
(Algorithm\,\ref{alg:recursivepart}), that is executed until a specified maximum
...
In case of draws, the assignment is made to the sub-set with less elements in order
...
The next step is similar to the previous one, but we now partition the set of probes
$\mathcal{P}$. Moreover, when we examine each probe $p_i \in \mathcal{P}$, we
compute the minimum weighted distance that any embedding of of $p_i$ can have to the
embeddings of the pivots $q'$ and $q''$, assigning $p_i$ to the sub-set $\mathcal{P}'$
or $\mathcal{P}''$ associated with the pivot with the minimum distance. This is done
with the OSPE algorithm described in Sect.\,\ref{sec:ospe}. In fact, a special version
is used since there is no information, at this moment, of the locations of the probes
or pivots.
...
The current partitioning depth determines if a horizontal or vertical partitioning is
performed. Since we only deal with rectangular regions, it may be necessary to move
a few probes from one partition to the other.

Each sub-region is then processed recursively. Once the maximum partitioning depth
$t_max$ is reached the maximum, the Row-epitaxial~\cite{KAHNG03A} algorithm is
called to place the probes of $\mathcal{P} \cup \mathcal{Q}$ in the region $R$.
Before that, however, all probes of $p_i \in \mathcal{P}$ are re-embedded optimally
...
order to ``synchronize'' any skewed embeddings. Also, it finds an embedding of
$p_i$ that has minimum conflict with the ``average'' probe assigned to $R$.
