...
instance, the PP generates on average 8\% less border length on $500\times 500$ chips
with $t_{max}=2$.}
...
\begin{table}
\label{tab:pp_affy_bl}
\caption{Total border length of chips produced by Pivot Partitioning (PP) using probe sets
of existing Affymetrix GeneChip arrays, with partitioning depths ranging from $t_{max}=0$
(no partitioning) to $t_{max}=8$. Placement is done by Row-epitaxial (with $Q = 20\,000$),
and is followed by the Sequential post-placement optimization.}
\centerline{\scriptsize{
\begin{tabular}{clrrlrrlrr}
& & \multicolumn{1}{c}{CQ} & \multicolumn{1}{c}{PP} & & \multicolumn{1}{c}{CQ} & \multicolumn{1}{c}{PP} & & \multicolumn{1}{c}{CQ} & \multicolumn{1}{c}{PP} \\
\noalign{\smallskip}
...
\begin{table}
\label{tab:pp_affy_ci}
\caption{Average conflict index of chips produced by Pivot Partitioning (PP) using probe sets
of existing Affymetrix GeneChip arrays, with partitioning depths ranging from $t_{max}=0$
(no partitioning) to $t_{max}=8$. Placement is done by Row-epitaxial (with $Q = 20\,000$),
and is followed by the Sequential post-placement optimization.}
\centerline{\scriptsize{
