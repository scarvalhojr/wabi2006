...
\caption{Ranges of values for both $\delta$ and $\omega$ on a typical 
  ...
  weighting function $\delta(p,p',t)$ for a probe~$p$ (shown in the center) and
  close neighbors $p'$, assuming that $p'$ is unmasked at step $t$. Right:
  position-dependent weights $\omega(p,t)$ on the y-axis for each value of
  $b_{p,t}$ on the x-axis, assuming that $p$ is masked at step $t$.}
...
Table~\ref{tab:pp_ci} shows similar results with the average conflict index.
For these experiments, we use a version of Row-epitaxial implemented for
conflict index minimization, which fills every spot with a probe~$p$
minimizing $\mathcal{C}(p)$.  For the post-placement optimization, we use the
Sequential algorithm with OSPE for conflict index minimization as described in
Sect.\,\ref{sec:ospe}. Computing the conflict index of a spot for every probe
candidate is not as straight forward as computing the Hamming distance between
a probe and its neighbors; thus both versions of Row-epitaxial and Sequential
for conflict index minimization are significantly slower. For this reason, we
set the limit on the number of probes considered by the Row-epitaxial to $Q =
2\,000$.
...
\bibitem{CELA98} \c{C}ela,E. (1998) {\it The Quadratic Assignment Problem: Theory
