...
We first compare how the Pivot Partitioning (PP) performs in comparison to the
Centroid-based Quadrisection (CQ) in terms of border length
(Table~\ref{tab:pp_x_cq_bl}).
Equivalence of partitioning depths.
This is sync embeddings, async are likely to be better.
...
$t_{max}=2$ to $t_{max}=6$ for PP. The data shows the total border length of chips
produced by the CQ (extracted from~\cite{KAHNG03B}), and the results of using the PP on
similar input, as percentage of the reduction in border length compared to the CQ.
Both partitionings use Row-epitaxial for the placement with $Q = 20\,000$, and are followed
by the Sequential post-placement optimization.}
...
\noalign{\smallskip}
100 & & \   393,218 & \ 0.16\% & & \   399,312 & \ -1.98\% & & \   410,608 & \ -2.87\% \\
200 & & \ 1,524,803 & \        & & \ 1,545,825 & \         & & \ 1,573,096 & \         \\
300 & & \ 3,493,552 & \        & & \ 3,413,316 & \         & & \ 3,434,964 & \         \\
500 & & \ 9,546,351 & \ 7.87\% & & \ 9,355,231 & \  4.67\% & & \ 9,307,510 & \  1.03\% \\
\noalign{\smallskip}
\hline
\end{tabular}}}
\end{table}
...
\ignore{
\multicolumn{10}{c}{Experiments for PP were conducted on a Sun Fire V1280 server with 900Mhz UltraSparc III+ processors and 96 Gb of RAM under similar load balances.}
}
